// junior/components/Sidebar.jsx
import { NavLink, useNavigate } from "react-router-dom";
import { MdMenuOpen } from "react-icons/md";
import { FaUserCircle } from "react-icons/fa";
import { LayoutDashboard, CheckSquare, Briefcase, FileText, LogOut, Scale } from 'lucide-react';
  ...
  return (
    <div className={`fixed left-0 top-0 shadow-md h-screen p-2 flex flex-col duration-500 bg-white text-gray-800 z-50 ${open ? 'w-60' : 'w-16'}`}>
      <div className='px-3 py-2 h-20 flex justify-between items-center'>
        <Scale
          className={`${open ? 'w-10 h-10 opacity-100' : 'w-0 h-0 opacity-0'} text-purple-700 transition-all duration-500`}
        />
        <div>
          <MdMenuOpen
            size={36}
      ...
      <div className='flex items-center gap-2 px-3 py-2 border-t cursor-pointer hover:bg-gray-200 rounded-md transition' onClick={handleProfile}>
        ...
        <div className={`leading-5 ${!open && 'w-0 translate-x-24'} duration-500 overflow-hidden`}>
          <p className="font-medium">{user.name || 'Junior Advocate'}</p>
          <span className='text-xs text-gray-600'>{user.email || 'email'}</span>
        </div>
      </div>
    </div>
  );
