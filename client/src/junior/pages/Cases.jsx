// junior/pages/Cases.jsx - WITH CASE DETAILS BUTTON
import { useState, useEffect } from 'react';
import { Briefcase, Calendar, User, MapPin, MessageCircle, Send, X, Eye, FileText, Clock } from 'lucide-react';
...
const JuniorCases = () => {
  ...
  const StatusBadge = ({ status }) => {
    ...
    return (
      <span className={`px-3 py-1 text-xs font-semibold rounded-full ${colors[status] || 'bg-gray-100 text-gray-700'}`}>
    ...
    <div className="p-4 md:p-6 space-y-6">
      ...
      <div className="grid gap-4">
        {cases.length > 0 ? cases.map((caseItem) => (
          <div key={caseItem._id} className="bg-white p-6 rounded-xl shadow-lg hover:shadow-xl transition">
            <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
              <div className="flex-1">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <h3 className="text-xl font-bold text-gray-900">{caseItem.title}</h3>
                    <p className="text-sm text-gray-600 mt-1">{caseItem.caseNumber}</p>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {caseItem.court && (
                    <div className="flex items-center gap-2 text-sm text-gray-700">
                      ...
                      <span>Court: <span className="font-semibold">{caseItem.court}</span></span>
                    </div>
                  )}
                  ...
                  {caseItem.advocateId && (
                    <div className="flex items-center gap-2 text-sm text-gray-700">
                      ...
                      <span>Lead: <span className="font-semibold">
                        {caseItem.advocateId?.name || 'Advocate'}
                      </span></span>
                    </div>
                  )}
                  ...
                  {caseItem.hearingDate && (
                    <div className="flex items-center gap-2 text-sm text-gray-700">
                      ...
                      <span>Hearing: <span className="font-semibold">
              ...
              <div className="flex flex-col gap-2">
                ...
                <button
                  onClick={() => openMessagesModal(caseItem)}
                  className="px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition flex items-center gap-2 whitespace-nowrap shadow-md hover:shadow-lg"
                >
      ...
      {showDetailsModal && selectedCase && (
        ...
        >
          ...
          <div className="relative bg-white rounded-xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
            {/* Header */}
            <div className="flex items-center justify-between p-6 border-b bg-linear-to-r from-blue-600 to-blue-700 text-white rounded-t-xl sticky top-0">
              <div>
            ...
            <div className="p-6 space-y-6">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-gray-50 p-4 rounded-lg">
                  ...
                  <div>
                    <p className="text-xs text-gray-500">Case Created</p>
              ...
              {selectedCase.advocateId && (
                <div>
                  ...
                  <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
                    <p className="font-bold text-gray-900 text-lg">{selectedCase.advocateId.name}</p>
                    <p className="text-sm text-gray-600">{selectedCase.advocateId.email}</p>
                    <span className="inline-block mt-2 px-2 py-1 bg-blue-600 text-white text-xs font-semibold rounded">
                      {selectedCase.advocateId.role?.toUpperCase()}
                    </span>
                  </div>
                </div>
              )}
                ...
                <div>
                  ...
                  <div className="bg-green-50 p-4 rounded-lg border border-green-200">
                    <p className="font-bold text-gray-900 text-lg">
                      {selectedCase.clientId.userId?.name || 'N/A'}
                    </p>
                    <p className="text-sm text-gray-600">
                      {selectedCase.clientId.userId?.email || 'N/A'}
                    </p>
                    {selectedCase.clientId.notes && (
                      <div className="mt-2 p-2 bg-white rounded">
                        <p className="text-xs text-gray-500">Notes:</p>
                        <p className="text-sm text-gray-700">{selectedCase.clientId.notes}</p>
                      </div>
                    )}
                  </div>
                </div>
              ...
              {/* Assigned Juniors Count */}
              {selectedCase.assignedJuniors && selectedCase.assignedJuniors.length > 0 && (
                <div>
                  <h3 className="text-sm font-semibold text-gray-600 uppercase mb-2">Team</h3>
                  <div className="bg-purple-50 p-3 rounded-lg border border-purple-200">
                    <p className="text-sm text-gray-700">
                      <span className="font-semibold">{selectedCase.assignedJuniors.length}</span> Junior Advocate(s) assigned
                    </p>
                  </div>
                </div>
              )}
      ...
      {/* Messages Modal (previous code remains the same) */}
      {showMessagesModal && selectedCase && (
        ...
        >
          ...
          <div className="relative bg-white rounded-xl shadow-2xl max-w-4xl w-full h-[85vh] flex flex-col">
            <div className="flex items-center justify-between p-6 border-b bg-linear-to-r from-purple-600 to-purple-700 text-white rounded-t-xl">
              <div>
                <h2 className="text-xl font-bold">{selectedCase.title}</h2>
                <p className="text-sm text-purple-100">{selectedCase.caseNumber}</p>
              </div>
              <button 
                onClick={closeMessagesModal} 
                ...
                messages.map((msg) => {
                  ...
                  return (
                    <div key={msg._id} className={`flex ${isSent ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-[70%] ${isSent ? 'bg-purple-600 text-white' : 'bg-white text-gray-900'} rounded-lg p-4 shadow-md`}>
                        <div className="flex items-center gap-2 mb-1">
                          <p className={`text-xs font-semibold ${isSent ? 'text-purple-100' : 'text-gray-600'}`}>
                            {isSent ? 'You' : msg.sender?.name || 'Unknown'}
                          </p>
                          <span className={`text-xs ${isSent ? 'text-purple-200' : 'text-gray-500'}`}>
                            ({msg.sender?.role || 'unknown'})
                          </span>
                        </div>
                        <p className="wrap-break-words whitespace-pre-wrap">{msg.content}</p>
                        <p className={`text-xs mt-2 ${isSent ? 'text-purple-200' : 'text-gray-500'}`}>
              ...
              <div className="space-y-3">
                <div className="bg-purple-50 p-3 rounded-lg border border-purple-200">
                  <p className="text-sm text-gray-700">
                    ...
                    <span className="text-purple-700 font-semibold">
                      {selectedCase.advocateId?.name || 'Lead Advocate'}
                    </span>
                    {' '}(Advocate)
                  </p>
                </div>
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="px-6 py-3 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 font-medium shadow-md hover:shadow-lg"
                  >
                    <Send size={20} />
