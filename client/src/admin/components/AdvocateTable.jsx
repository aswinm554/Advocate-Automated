            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-100">
                <tr>
                  ...
                  <th className="p-3 text-left font-semibold">Rejection Reason</th>
                </tr>
              </thead>
              <tbody>
                  ...
                  filteredAdvocates.map((a) => (
                    <tr key={a.id} className="hover:bg-gray-50 transition">
                      ...
                      <td className="p-3 text-center">
                        <span className={`inline-block px-3 py-1 text-xs rounded-full font-medium ${a.status === 'approved'
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-100">
                <tr>
                  ...
                  <th className="p-2 text-left font-semibold">Rejection Reason</th>
                    ...
                    <tr key={a.id} className="hover:bg-gray-50 transition">
                      <td className="p-2">
                        ...
                        <div className="text-xs text-gray-500">
                          {a.specialization} • {a.experience} yrs
                        </div>
                      </td>
                      ...
                      <td className="p-2 text-center text-gray-700 text-sm">
                        {a.licenseNumber}
                      </td>

                      <td className="p-2 text-center">
                        <span className={`inline-block px-2 py-1 text-xs rounded-full font-medium ${a.status === 'approved'
                            ? 'bg-green-100 text-green-700'
                            : a.status === 'pending'
                              ? 'bg-yellow-100 text-yellow-700'
                              : 'bg-red-100 text-red-700'
              ...
              filteredAdvocates.map((a) => (
                <div key={a.id} className="rounded-lg p-3 sm:p-4 bg-gray-50 hover:bg-gray-100 transition">
                  <div className="flex justify-between items-start mb-2 sm:mb-3">
                    ...
                    <span className={`shrink-0 px-2 sm:px-3 py-1 text-xs rounded-full font-medium whitespace-nowrap ${a.status === 'approved'
                      ? 'bg-green-100 text-green-700'
                      : a.status === 'pending'
                        ? 'bg-yellow-100 text-yellow-700'
                        : 'bg-red-100 text-red-700'
